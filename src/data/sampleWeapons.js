...
const weapons = RAW_WEAPONS.map((weapon) => {
  ...
  const legacyRarity = normalizeRarity(legacy.rarity);
  const specifiedRarity = normalizeRarity(weapon.rarity);
  const fallbackRarity = FALLBACK_RARITY_BY_CATEGORY[category] || 'common';

  return normalizeWeapon({
    id: slugify(weapon.name),
    name: weapon.name,
    category,
    rarity: legacyRarity || specifiedRarity || fallbackRarity,
